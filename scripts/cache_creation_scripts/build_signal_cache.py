"""
Stage 1: Build cache of bars, profiles, and signals.
Run this once (30 mins), then test strategies instantly.
"""
...
SIGNAL_CACHE_DIR = CACHE_DIR / "signal_cache"
...
def generate_entry_signals(all_signals, profiles, proximity_ticks=40, start_time_et="09:30", end_time_et="11:00"):
    """Generate entry signals near VAL/VAH with matching cluster bias, filtered to RTH."""
    entry_signals = []
    proximity_points = proximity_ticks * TICK_SIZE

    for signal in all_signals:
        # Filter to RTH trading hours
        confirm_time_et = signal["confirm_time"].tz_convert(ET)
        hour_min = confirm_time_et.strftime("%H:%M")
        if hour_min < start_time_et or hour_min >= end_time_et:
            continue
        ...
        if abs(signal_price_ref - val_price) <= proximity_points:
            near_val = True
            # Clip VAL cluster to profile range
            # Can't always go 25 points below VAL (might be outside profile)
            # But can always go 25 points above VAL
        ...
        if abs(signal_price_ref - vah_price) <= proximity_points:
            near_vah = True
            # Clip VAH cluster to profile range
            # Can always go 25 points below VAH
            # But can't always go 25 points above VAH (might be outside profile if day expanding)
...
def load_data_fast(start_date, end_date):
    ...
    Checks (in order):
      1. Range-named parquet: NQ_c_0_mbp-1_{start}_{end}.parquet
      2. Range-named dbn:     NQ_c_0_mbp-1_{start}_{end}.dbn
      3. Single-day dbn files for each date in range (e.g. 2026-04-07.dbn)
    """
    data_dir = CACHE_DIR
    ...
    # Fallback: load individual single-day dbn files covering the range
...
def process_and_cache_single_day(date_obj):
    ...
    try:
        ...
        # Generate entry signals (with RTH filter)
...
if __name__ == '__main__':
    print("="*80)
    print("BUILDING SIGNAL CACHE - RUN ONCE, TEST FOREVER")
    print("="*80)
    print()
    ...
    if existing_cache:
        ...
        response = input("Delete existing cache and rebuild? (y/n): ").lower()
        if response == 'y':
            for f in existing_cache:
                f.unlink()
            print("Existing cache deleted")
        else:
    ...
    print()
    print("Now run: python scripts/test_from_cache.py")
    print()
