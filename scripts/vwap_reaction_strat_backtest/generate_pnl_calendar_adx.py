"""
Generate P&L Calendar HTML for VWAP Reaction strategy with ADX 15-30 filter.
No time filter. SL 1.0x / TP 2.0x ATR.
MNQ/NQ switcher with 1-9 contract slider.
"""
...
def collect_daily_trades(adx_lookup):
...
<html>
<head>
    <title>VWAP Reaction Strategy - P&L Calendar (ADX {int(ADX_LOW)}-{int(ADX_HIGH)})</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="subtitle">ADX {int(ADX_LOW)}-{int(ADX_HIGH)} Filter | SL 1.0x ATR / TP 2.0x ATR | {total_trades} trades | {win_rate:.1f}% WR</div>

        <div class="strategy-note">
            <h3>Strategy: VWAP Reaction Continuation (ADX Filtered)</h3>
            <p><strong>Filter:</strong> ADX {int(ADX_LOW)}-{int(ADX_HIGH)} (14-period on 5-min bars)</p>
            <p><strong>Entry:</strong> Absorption signal + confirmation at VWAP zone (+/-3 pts), 5-min bias confirms direction</p>
            <p><strong>Stop:</strong> 1.0x ATR | <strong>Target:</strong> 2.0x ATR</p>
            <p><strong>Session:</strong> 7 PM - 4:00 PM ET (force close 4:58 PM)</p>
...
def main():
    ...
    print("Collecting daily trades with ADX filter...")
    daily = collect_daily_trades(adx_lookup)
    print(f"  {sum(len(v) for v in daily.values())} trades across {len(daily)} trading days")

    output = Path("results/html/vwap_reaction_pnl_calendar_adx.html")
