...
In-sample / out-of-sample test for VWAP reaction strategy with filters:
  - Regime: choppy or moderate only
...
TRADES_CSV = f"{DATA_DIR}/vwap_reaction_regime_trades.csv"
...
def load_and_filter():
    ...
    # Regime filter: choppy or moderate
    df = df[df["regime"].isin(["choppy", "moderate"])].copy()
...
def report(df, label):
    ...
    for r in ["choppy", "moderate"]:
        ...
        spnl = sub["pnl_dollars"].sum()
        swr = len(sub[sub["pnl_dollars"] > 0]) / sn * 100
        print(f"  {r.capitalize():<10s}:     {sn} trades | WR {swr:.1f}% | ${spnl:+,.0f}")
...
def main():
    ...
    print(f"Filters: choppy/moderate regime | 9:30am-4pm ET | no lunch (12-1pm)")
