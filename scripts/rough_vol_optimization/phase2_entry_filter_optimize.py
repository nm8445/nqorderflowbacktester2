"""
Phase 2: Optimize HIGH_Z x EMA_LEN with 60/40 IS/OOS split.
Locked: H=0.40, KERNEL=80, NORM=250, ZLOOK=100, SL=2, TP=1.2, session 6:45-15:45 ET.
"""
import pickle
import numpy as np
import pandas as pd
...
NORM_LEN = 250; Z_LOOKBACK = 100
ATR_LEN = 14; ATR_SL = 2.0; ATR_TP = 1.2
ET = "America/New_York"; POINT_VALUE = 20.0
SS = "06:45"; SE = "15:45"; MT = 5
MART_STREAK = 1; MART_MULT = 1.5; MART_MAX_DOUBLES = 4

# Grid
HIGH_ZS = [0.5, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0]
EMA_LENS = [15, 20, 30, 40, 50, 75, 100, 150, 200]
...
def precompute(df):
    """Precompute signal (z_vol) and ATR — these don't change across the grid."""
    ...
    return z_vol, atr_vals, n


def backtest(closes_v, highs_v, lows_v, z_vol, ema, atr_vals, idx_list, n, high_z, date_mask_is, date_mask_oos):
    results = {}
    for label, mask in [("IS", date_mask_is), ("OOS", date_mask_oos)]:
        ...
        for y in unique_years:
            ym = years_arr == y
            yp = pnls[ym]; yw = yp[yp > 0]; yl = yp[yp < 0]
        ...
        results[label] = {"trades": len(trades), "pf": pf, "wr": wr, "pnl": pnls.sum(),
                          "dd": dd, "tpy": tpy, "year_pfs": year_pfs, "losing_years": losing_years}
    return results
...
def main():
    ...
    z_vol, atr_vals, n = precompute(df)
    ...
    for ema_len in EMA_LENS:
        ema_cache[ema_len] = closes_s.ewm(span=ema_len, adjust=False).mean().values

    print(f"\nGrid: HIGH_Z {HIGH_ZS} x EMA_LEN {EMA_LENS}")
    print(f"Total combos: {len(HIGH_ZS) * len(EMA_LENS)}")
    print(f"Locked: NORM={NORM_LEN}, ZLOOK={Z_LOOKBACK}\n")

    all_results = []
    total = len(HIGH_ZS) * len(EMA_LENS)
    done = 0; t0 = time.time()

    for high_z in HIGH_ZS:
        for ema_len in EMA_LENS:
            ema = ema_cache[ema_len]
            res = backtest(closes_v, highs_v, lows_v, z_vol, ema, atr_vals, idx_list, n, high_z, date_mask_is, date_mask_oos)
            is_r = res["IS"]; oos_r = res["OOS"]
            all_results.append({"high_z": high_z, "ema_len": ema_len, "is": is_r, "oos": oos_r})
            done += 1
            if done % 9 == 0:
    ...
    print(f"{'HI_Z':>5} {'EMA':>4} | {'IS Trd':>7} {'IS PF':>6} {'IS WR':>6} {'IS PnL':>10} {'IS DD':>10} {'IS LY':>5} | "
          f"{'OOS Trd':>7} {'OOS PF':>7} {'OOS WR':>7} {'OOS PnL':>10} {'OOS DD':>10} {'OOS LY':>6}")
    print("-" * 125)
    for r in valid[:25]:
        is_r = r["is"]; oos_r = r["oos"]
        print(f"{r['high_z']:>5.1f} {r['ema_len']:>4} | "
    ...
    print(f"{'HI_Z':>5} {'EMA':>4} | {'IS PF':>6} {'IS WR':>6} {'IS PnL':>10} | {'OOS PF':>7} {'OOS WR':>7} {'OOS PnL':>10} {'OOS DD':>10}")
    print("-" * 90)
    ...
    print(f"{'HI_Z':>5} {'EMA':>4} | {'IS PF':>6} {'OOS PF':>7} {'AVG':>5} | {'IS Trd':>7} {'OOS Trd':>7} {'Total PnL':>10} {'Worst DD':>10} {'IS WR':>6} {'OOS WR':>7}")
    print("-" * 105)
