"""
NQGRAY config test — H=0.4, KERNEL=80, NORM=201, ZLOOK=100, HIGH_Z=1.1, LOW_Z=-1,
EMA=40, ATR_SL=2, ATR_TP=1.2, Session 6:45AM-3:45PM ET.
Trend-only AND trend+mean-revert comparison. Martingale streak=1.
"""
import pickle
import numpy as np
import pandas as pd
...
NORM_LEN = 300; Z_LOOKBACK = 100; HIGH_Z = 1.5; LOW_Z = 1.0; EMA_LEN = 40; ATR_LEN = 14
...
def backtest(df, include_mr=False):
    trades = []
    trade_types = []
    pos = 0; ep = 0; sl = 0; tp = 0; d = ""; cd = None; dt = 0
    qty = 1; loss_streak = 0; cur_type = ""
    ...
    for i in range(len(df)):
        ...
        if pos != 0 and not ins and hm >= SE:
            ...
            trades.append(pnl); trade_types.append(cur_type)
        ...
        if pos == 0 and dt < MT:
            ...
            if z > HIGH_Z and cl > ema:
                ...
                entered = True; cur_type = "TREND"
            elif z > HIGH_Z and cl < ema:
                pos = -1; d = "short"; ep = cl; sl = cl + ATR_SL * atr_v; tp = cl - ATR_TP * atr_v
                entered = True; cur_type = "TREND"
            if not entered and include_mr and z < -LOW_Z:
                if cl < ema:
                    pos = 1; d = "long"; ep = cl; sl = cl - ATR_SL * atr_v; tp = cl + ATR_TP * atr_v
                    entered = True; cur_type = "MR"
                elif cl > ema:
                    pos = -1; d = "short"; ep = cl; sl = cl + ATR_SL * atr_v; tp = cl - ATR_TP * atr_v
                    entered = True; cur_type = "MR"
            if entered: dt += 1

    return trades, trade_types


def report(label, trades, trade_types=None):
    ...
    if len(w): print(f"  Avg Win: ${w.mean():+,.0f}  Avg Loss: ${l.mean():+,.0f}" if len(l) else f"  Avg Win: ${w.mean():+,.0f}")

    if trade_types:
        types_arr = np.array(trade_types)
        for t in ["TREND", "MR"]:
            mask = types_arr == t
            if mask.sum() == 0: continue
            tp = pnls[mask]
            tw = tp[tp > 0]; tl = tp[tp < 0]
            tpf = tw.sum() / abs(tl.sum()) if len(tl) else 99
            twr = 100 * len(tw) / len(tp)
            print(f"    {t}: {len(tp)} trades, PF {tpf:.2f}, WR {twr:.1f}%, PnL ${tp.sum():+,.0f}")


def main():
    print("NQGRAY Config Test (Z_LOOKBACK=100)")
    print(f"H={H}, KERNEL={KERNEL_LEN}, NORM={NORM_LEN}, ZLOOK={Z_LOOKBACK}")
    print(f"HIGH_Z={HIGH_Z}, LOW_Z=-{LOW_Z}, EMA={EMA_LEN}, ATR_SL={ATR_SL}, ATR_TP={ATR_TP}")
    print(f"Session: {SS}-{SE} ET, Martingale streak={MART_STREAK}")
    ...
    # Trend only
    t1, tt1 = backtest(df, include_mr=False)
    report("TREND ONLY", t1)

    # Trend + Mean-Revert
    t2, tt2 = backtest(df, include_mr=True)
    report("TREND + MEAN-REVERT", t2, tt2)
