"""
z_vol exit strategy — NO Hurst filter, no flip.
Norm=201, ZLook=23, HIGH_Z=1.1, EMA=40, SL=2.0x ATR
No TP — exit when z_vol <= -1.0 or SL hit. Force close 15:27.
"""
import pickle
import numpy as np
import pandas as pd
...
def main():
    ...
    # Backtest — run flat and mart
    for use_mart, mlabel in [(False, "FLAT"), (True, "MARTINGALE")]:
        print(f"\n{'='*60}\n{mlabel}\n{'='*60}", flush=True)
        trades = []
        ...
        for i in range(n):
            ...
            if pos != 0:
                ...
                if d == "long" and row["low"] <= sl:
                    pnl = (sl - ep) * POINT_VALUE * qty
                    trades.append({"pnl": round(pnl, 2), "date": str(td), "exit": "SL"})
                    if pnl > 0: loss_streak = 0
                    else: loss_streak += 1
                    if use_mart and loss_streak >= MART_STREAK:
                        steps = min(loss_streak - MART_STREAK + 1, MART_MAX_DOUBLES)
            ...
            # Entry: z > HIGH_Z with EMA filter
            if pos == 0 and dt < MT:
        ...
        print(f"\nConfig: Norm={NORM_LEN} ZLook={Z_LOOKBACK} HIGH_Z={HIGH_Z} EMA={EMA_LEN} SL={ATR_SL}x | Exit: z<={LOW_Z} or SL | NO HURST")
