"""
NQGRAY April trade log — H=0.4, KERNEL=80, NORM=300, ZLOOK=100, HIGH_Z=1.5,
EMA=40, ATR_SL=2, ATR_TP=1.2, Session 6:45AM-3:45PM ET. Trend-only, streak=1.
"""
import pickle
import numpy as np
import pandas as pd
...
def main():
    ...
    # Backtest full history, log target month
    trades = []
    pos = 0; ep = 0; sl = 0; tp = 0; d = ""; cd = None; dt = 0
    qty = 1; loss_streak = 0
    entry_time = None; entry_z = 0
    ...
    for i in range(len(df)):
        ...
        if pos != 0 and not ins and hm >= SE:
            ...
            if td.month == TARGET_MONTH and td.year == TARGET_YEAR:
                trades.append({"date": td, "action": "BUY" if d == "long" else "SELL",
                    "qty": qty, "entry": ep, "exit": row["close"], "pnl": pnl,
                    "reason": "SESSION", "entry_time": entry_time, "exit_time": idx, "z": entry_z})
        ...
        if pos != 0:
            ...
            if xp:
                ...
                if td.month == TARGET_MONTH and td.year == TARGET_YEAR:
                    trades.append({"date": td, "action": "BUY" if d == "long" else "SELL",
                        "qty": qty, "entry": ep, "exit": xp, "pnl": pnl,
                        "reason": xr, "entry_time": entry_time, "exit_time": idx, "z": entry_z})
                if pnl > 0: loss_streak = 0
                else: loss_streak += 1
                if loss_streak >= MART_STREAK:
                    steps = min(loss_streak - MART_STREAK + 1, MART_MAX_DOUBLES)
        ...
        if pos == 0 and dt < MT:
            if z > HIGH_Z and cl > ema:
                ...
                dt += 1; entry_time = idx; entry_z = z
            elif z > HIGH_Z and cl < ema:
                pos = -1; d = "short"; ep = cl; sl = cl + ATR_SL * atr_v; tp = cl - ATR_TP * atr_v
                dt += 1; entry_time = idx; entry_z = z

    print(f"{'Symbol':<6} {'Action':<6} {'Qty':>4} {'Entry':>12} {'Exit':>12} {'P&L':>12} {'Duration':<16} {'Entry Time':<22} {'Exit Time':<22} {'Exit Rsn':<8} {'z_vol':>6}")
    print("-" * 140)
    current_date = None; day_pnl = 0.0; month_pnl = 0.0
    for t in trades:
        ...
        print(f"{'NQ':<6} {t['action']:<6} {t['qty']:>4} ${t['entry']:>10,.2f} ${t['exit']:>10,.2f} ${t['pnl']:>+10,.2f} {dur_str:<16} {entry_str:<22} {exit_str:<22} {t['reason']:<8} {t['z']:>+6.2f}")
        day_pnl += t["pnl"]; month_pnl += t["pnl"]
    if current_date is not None:
        print(f"{'':>42} Day total: ${day_pnl:>+10,.2f}")
    print()
    print("=" * 80)
    ...
    if trades:
        print(f"April {TARGET_YEAR} Summary: {len(trades)} trades | PF={pf:.2f} | PnL=${month_pnl:+,.2f} | WR={100*len(wins)/len(trades):.1f}%")
    else:
        print("No trades")
