"""
Phase 4: Optimize session start/end and MAX_TRADES with 60/40 IS/OOS split.
Locked: H=0.40, KERNEL=80, NORM=250, ZLOOK=100, HIGH_Z=1.5, EMA=50,
        SL=1.5, TP=2.5, ATR_LEN=14, session TBD, streak=1.
"""
import pickle
import numpy as np
import pandas as pd
...
# Grid
SESSION_STARTS = ["05:00", "06:00", "06:45", "07:30", "08:00", "08:30", "09:00", "09:30"]
SESSION_ENDS = ["14:00", "14:45", "15:00", "15:30", "15:45", "16:00"]
MAX_TRADES_LIST = [3, 5, 7, 10, 15]
...
def backtest(closes_v, highs_v, lows_v, z_vol, ema, atr_vals, idx_list, hm_list, n,
             ss, se, mt, date_mask_is, date_mask_oos):
    results = {}
...
def main():
    ...
    total = len(SESSION_STARTS) * len(SESSION_ENDS) * len(MAX_TRADES_LIST)
    print(f"\nGrid: starts {SESSION_STARTS}")
    print(f"       ends  {SESSION_ENDS}")
    print(f"       max_trades {MAX_TRADES_LIST}")
    print(f"Total combos: {total}")
    print(f"Locked: NORM={NORM_LEN}, ZLOOK={Z_LOOKBACK}, HIGH_Z={HIGH_Z}, EMA={EMA_LEN}, SL={ATR_SL}, TP={ATR_TP}, ATR={ATR_LEN}\n")
        ...
        for se in SESSION_ENDS:
            ...
            for mt in MAX_TRADES_LIST:
                res = backtest(closes_v, highs_v, lows_v, z_vol, ema, atr_vals, idx_list, hm_list, n,
                               ss, se, mt, date_mask_is, date_mask_oos)
                all_results.append({"ss": ss, "se": se, "mt": mt, "is": res["IS"], "oos": res["OOS"]})
                done += 1
                if done % 30 == 0:
    ...
    print(f"{'Start':>6} {'End':>6} {'MT':>3} | {'IS Trd':>7} {'IS PF':>6} {'IS WR':>6} {'IS PnL':>10} {'IS DD':>10} {'IS LY':>5} | "
          f"{'OOS Trd':>7} {'OOS PF':>7} {'OOS WR':>7} {'OOS PnL':>10} {'OOS DD':>10} {'OOS LY':>6}")
    print("-" * 130)
    for r in valid[:25]:
        is_r = r["is"]; oos_r = r["oos"]
        print(f"{r['ss']:>6} {r['se']:>6} {r['mt']:>3} | "
    ...
    print(f"{'Start':>6} {'End':>6} {'MT':>3} | {'IS PF':>6} {'IS WR':>6} {'IS PnL':>10} | {'OOS PF':>7} {'OOS WR':>7} {'OOS PnL':>10} {'OOS DD':>10}")
    print("-" * 100)
    ...
    print(f"{'Start':>6} {'End':>6} {'MT':>3} | {'IS PF':>6} {'OOS PF':>7} {'AVG':>5} | {'IS Trd':>7} {'OOS Trd':>7} {'Total PnL':>10} {'Worst DD':>10} {'IS WR':>6} {'OOS WR':>7}")
    print("-" * 115)
