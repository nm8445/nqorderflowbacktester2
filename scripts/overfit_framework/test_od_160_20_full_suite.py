"""Full 5-test overfit framework for the Overnight Drift green_base=160 green_decay=2.0 variant.

Variant of test_n25_full_suite.py — same engine, same yellow_suppress=25 base,
but with the new green TP config that came out of sweep_green_base_decay.py:
  - green_base   = 160 (vs locked 82.5)
  - green_decay  = 2.0  (vs locked 1.5)
  - all other params held at LOCKED_N25 values

Question: does the wider-green variant survive Tests 1-5 the way locked did?
"""
...
LOCKED_BASELINE = dict(CFG_160_20)
LOCKED_BASELINE["green_base"] = 82.5
LOCKED_BASELINE["green_decay"] = 1.5
...
# ---------------- Tests 2, 3, 4 (need trade-level dataframe) ----------------
def tests_2_3_4(trades, rng):
    ...
    out = []
    out.append(f"\n### Overnight Drift (green_base=160, green_decay=2.0)  ({len(pnl)} trades)")
    out.append("=" * 80)
...
def test_1_parameter_stability():
    base, _ = od_run(CFG_160_20)
    out = ["\n--- Test 1: Parameter Stability (1D sweeps around green_base=160, green_decay=2.0) ---"]
    out.append(f"  CFG 160/2.0: PnL=${base['pnl']:+,.0f}  PF={base['pf']:.3f}  MDD=${base['mdd']:+,.0f}  trades={base['trades']}")

    sweeps = {
        "green_base":       [130.0, 145.0, 155.0, 160.0, 170.0, 180.0, 200.0],
        "green_decay":      [1.0, 1.5, 1.75, 2.0, 2.25, 2.5, 3.0],
    ...
    for param, values in sweeps.items():
        ...
        rows = []
        for v in values:
            cfg = dict(CFG_160_20); cfg[param] = v
            m, _ = od_run(cfg)
            tag = "  *CENTER*" if v == CFG_160_20[param] else ""
...
def test_5_direction_permutation(n_workers=6):
    ...
    real_pnl = _od_simulate_random_dir(AllLong(), CFG_160_20)
    out.append(f"  Real PnL (all long, no-mart at 1c): ${real_pnl:+,.0f}")
    args = [(s, CFG_160_20) for s in range(N_PERMS)]
    t0 = time.time()
    ...
    np.save(RESULTS / "od_160_20_perm_pnls.npy", pnls)
    return "\n".join(out)
...
def main():
    ...
    print("\n[1/3] Generating trade log for 160/2.0 config...")
    base_stats, trades = od_run(CFG_160_20)
    ...
    print("\n[3/3a] Test 1 parameter stability (engine reruns)...")
    ...
    final = "\n".join([
        "=" * 80,
        "OVERFIT FRAMEWORK — Overnight Drift green_base=160, green_decay=2.0",
        "=" * 80,
        out_234, out_1, out_5,
    ])
    with open(RESULTS / "od_160_20_full_suite_summary.txt", "w", encoding="utf-8") as f:
        f.write(final)
    print("\n" + final)
    print(f"\nSaved -> {RESULTS / 'od_160_20_full_suite_summary.txt'}")
